...
impl BytesColumn {
    ...
    pub(crate) fn into_column_index(self) -> ColumnIndex {
        match self {
            BytesColumn::DictionaryEncoded(column) => column.into_column_index(),
            BytesColumn::Plain(column) => column.into_column_index(),
        }
    }
...
impl StrColumn {
    ...
    pub(crate) fn into_column_index(self) -> ColumnIndex {
        match self {
            StrColumn::DictionaryEncoded(column) => column.into_column_index(),
            StrColumn::Plain(column) => column.into_column_index(),
        }
    }
