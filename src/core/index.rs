...
use crate::tokenizer::{BoxTokenizer, TokenizerManager};
...
impl Index {
    ...
    pub fn tokenizer_for_field(&self, field: Field) -> Result<BoxTokenizer> {
        let field_entry = self.schema.get_field_entry(field);
        let field_type = field_entry.field_type();
        let tokenizer_manager: &TokenizerManager = self.tokenizers();
        let tokenizer_name_opt: Option<BoxTokenizer> = match field_type {
