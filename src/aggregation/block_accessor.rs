...
impl ColumnBlockAccessor {
    ...
    pub(crate) fn fetch_source_block(
        ...
        match source {
            ...
            AggregationValueSource::Virtual(id) => {
                self.fetch_virtual_block(docs, *id, runtimes, None, false)
            }
        }
    }
    ...
    fn fetch_virtual_block(
        ...
        // Values and doc ids are appended one slot at a time below.
        self.val_cache.reserve(docs.len());
        self.docid_cache.reserve(docs.len());
        if missing.is_some() && !ordered {
            self.missing_docids_cache.reserve(docs.len());
        }
    ...
    pub(crate) fn fetch_source_block_with_missing(
        ...
        match source {
            ...
            AggregationValueSource::Virtual(id) => {
                self.fetch_virtual_block(docs, *id, runtimes, missing, false)
            }
        }
    }
    ...
    pub(crate) fn fetch_source_block_with_missing_unique_per_doc(
        ...
        match source {
            ...
            AggregationValueSource::Virtual(id) => {
                ...
                self.fetch_virtual_block(docs, *id, runtimes, missing, ordered)
            }
        }
    }
