...
fn virtual_buffer_reset_alignment_sparse_repeats_and_owned_identity() -> crate::Result<()> {
    ...
    for docs in [vec![4, 7], vec![0, 1, 3, 5], vec![1], vec![4, 7], vec![]] {
        data.column_block_accessor.fetch_source_block(
            &docs,
            &source,
            &mut data.value_sources,
            &mut data.context.limits,
        )?;
...
fn virtual_direct_constructor_flush_failure_and_framework_errors() -> crate::Result<()> {
    ...
    // Setup succeeds; framework-owned output growth fails at collection and is latched to harvest.
    let context = AggContextParams {
        limits: AggregationLimitsGuard::new(Some(2_000), None),
        ..Default::default()
    };
    let mut collector = AggregationSegmentCollector::from_agg_req_and_reader_with_virtual_columns(
        &aggs, segment, 0, &context, &registry,
    )?;
    let docs: Vec<DocId> = (0..1024).collect();
    collector.collect_block(&docs); // limit fails before calling the evaluator
    assert!(collector.harvest().is_err());
    assert_eq!(calls.load(Ordering::Relaxed), 3);
