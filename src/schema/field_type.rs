...
mod tests {
    ...
    fn test_to_string_coercion() {
        ...
        let doc = Document::parse_json(&schema, r#"{"id": 100}"#).unwrap();
        assert_eq!(
            &Value::Str("100".to_string()),
            doc.get_first(text_field).unwrap()
        );

        let doc = Document::parse_json(&schema, r#"{"id": true}"#).unwrap();
        assert_eq!(
        ...
        let doc = Document::parse_json(&schema, r#"{"id": null}"#).unwrap();
        assert_eq!(
    ...
    fn test_to_number_no_coercion() {
        ...
        let schema = schema_builder.build();
        assert!(Document::parse_json(&schema, r#"{"u64": "100"}"#)
            .unwrap_err()
            .to_string()
            .contains("a u64"));

        assert!(Document::parse_json(&schema, r#"{"i64": "100"}"#)
            .unwrap_err()
            .to_string()
            .contains("a i64"));

        assert!(Document::parse_json(&schema, r#"{"f64": "100"}"#)
            .unwrap_err()
            .to_string()
            .contains("a f64"));
    }
