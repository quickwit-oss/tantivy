...
impl Collector for FacetCollector {
    ...
    fn set_segment(&mut self, segment_id: SegmentLocalId, reader: &SegmentReader) -> Result<()> {
       for facet_type in self.facets.iter_mut() {
            match facet_type {
                 &mut FacedType::FastFieldI64(ref mut fast_field_value_facet) => fast_field_value_facet.set_segment(segment_id, reader),
                 &mut FacedType::FastFieldU64(ref mut fast_field_value_facet) => fast_field_value_facet.set_segment(segment_id, reader)
            };
        };
        Ok(())
    }
    ...
    fn collect(&mut self, doc: DocId, score: Score) {
        for facet_type in self.facets.iter_mut() {
            match facet_type {
                 &mut FacedType::FastFieldI64(ref mut fast_field_value_facet) => fast_field_value_facet.collect(doc, score),
                 &mut FacedType::FastFieldU64(ref mut fast_field_value_facet) => fast_field_value_facet.collect(doc, score)
            }
        };
    }
}
...
mod tests {
    ...
    fn test_facet_collector_results() {
        ...
        let num_field_i64 = schema_builder.add_i64_field(
            "num_i64",
            schema::IntOptions::default()
                .set_fast()
                .set_indexed()
            );
        let num_field_u64 = schema_builder.add_u64_field(
            "num_u64",
            schema::IntOptions::default()
                .set_fast()
                .set_indexed()
        ...
        let mut facet_collector = FacetCollector::new(vec![FacedType::FastFieldI64(ffvf_i64), FacedType::FastFieldU64(ffvf_u64)]);
