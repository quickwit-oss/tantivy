...
// SAFETY: Every string pointer stored in `input_values` points into a buffer
// owned by `string_inputs`, whose allocation remains stable when the scorer is
// moved. `DocSet` access requires `&mut self`, so evaluation and buffer updates
...
impl DocSet for CalculatedPredicateScorer {
    ...
    fn seek_danger(&mut self, target: DocId) -> SeekDangerResult {
        ...
        // and string pointers remain valid because `string_inputs` is not mutated
        // during this call.
        ...
        if unsafe { result.as_bool() } == Some(true) {
            self.doc = target;
