...
pub struct Disjunction<TScorer, TScoreCombiner = DoNothingCombiner> {
    ...
    marker2: PhantomData<TScoreCombiner>,
...
impl<TScorer: Scorer, TScoreCombiner: ScoreCombiner> Disjunction<TScorer, TScoreCombiner> {
    pub fn new<T: IntoIterator<Item=TScorer>>(
        ...
        debug_assert!(minimum_matches_required > 1, "union scorer works better if just one matches required");
...
impl<TScorer: Scorer, TScoreCombiner: ScoreCombiner> DocSet for Disjunction<TScorer, TScoreCombiner> {
    // TODO: add comments for better understanding.
    fn advance(&mut self) -> DocId {
        let mut count = 0;
        while !self.is_end && count < self.minimum_matches_required {
            if let Some(mut lowest) = self.chains.pop() {
                let next = lowest.0.0.doc();
                if next != TERMINATED {
                    if self.doc == next {
                        lowest.0.0.advance();
                        count += 1;
                    } else if count < self.minimum_matches_required {
                        lowest.0.0.advance();
                        self.doc = next;
                        count = 1;
                    }
                    self.chains.push(lowest);
                }
            } else {
                self.doc = TERMINATED;
                self.is_end = true;
...
impl<TScorer: Scorer, TScoreCombiner: ScoreCombiner> Scorer for Disjunction<TScorer, TScoreCombiner> {
...
mod tests {
    ...
    fn conjunct<T: Ord + Copy>(arrays: &[Vec<T>], pass_line: usize) -> Vec<T>
    {
        ...
        counts.iter()
            .filter_map(|(&element, &count)| if count >= pass_line { Some(element) } else { None })
            .collect()
    }
