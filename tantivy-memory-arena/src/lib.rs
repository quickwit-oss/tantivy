...
//!
//! ```rust
//! # extern crate tantivy_memory_arena;
//! use tantivy_memory_arena::{MemoryArena, Addr};
//!
//! # fn main() {
//! let mut memory_arena = MemoryArena::new();
//!
//! // Let's store a `&[u8]`...
//! let addr_hello_world = memory_arena.store_bytes( b"hello_world");
//! // ... and fetch it back from the arena
//! let hello_world_retrieved = memory_arena.read_slice(addr_hello_world, 5);
//! assert_eq!(b"hello", hello_world_retrieved)
//! # }
//! ```
use std::mem;
use std::ptr;
...
impl MemoryArena {
    ...
    /// Allocates and writes the bytes in `data`.
    pub fn store_bytes<B: AsRef<[u8]>>(&mut self, data: B) -> Addr {
        let bytes = data.as_ref();
        let addr = self.allocate_space(bytes.len());
    ...
    /// *Assume the required space (mem::size_of::<Item>()) has been pre-allocated*
    ///
    /// ```rust
    /// extern crate tantivy_memory_arena;
    ///
    /// use tantivy_memory_arena::{MemoryArena, Addr};
    ///
    /// // any object implementing `Copy` can be stored
    /// #[derive(Copy, Clone)]
    /// struct Vector {
    ///     pub x: f32,
    ///     pub y: f32,
    ...
    ///     let addr = arena.store(Vector {x: 1f32, y: 0f32, z: 0f32});
    ///     // ...
    ///     let vec: Vector = unsafe { arena.read::<Vector>(addr) };
    /// }
    ...
    pub unsafe fn mutate<Item: Copy, Mutator: FnMut(&mut Item)>(&mut self, addr: Addr, mut mutator: Mutator) {
