...
const NUMERIC_NUM_VALUES: u64 = 1_000;
...
impl Distribution {
    ...
    fn num_values(self) -> u64 {
        match self {
            Distribution::Rand => NUMERIC_NUM_VALUES,
            Distribution::Asc => NUMERIC_NUM_VALUES,
        }
    }
...
const ASC_NOISE_OFFSET: u64 = NUMERIC_NUM_VALUES / 2;
...
impl NumericFields {
    ...
    fn add_to(&self, doc: &mut TantivyDocument, rng: &mut StdRng, doc_id: usize) {
        // The fast and non-fast variants must hold identical data to be comparable.
        let rand = rng.random_range(0u64..NUMERIC_NUM_VALUES);
        // A perfectly ascending field selects the blockwise-linear codec. Add deterministic
        // high/low noise across the value domain so bitpacking wins, without consuming RNG state
        // or losing the coarse ascending trend.
        let noise = ((doc_id as u64).wrapping_mul(0x9E37_79B9_7F4A_7C15) >> 63) * ASC_NOISE_OFFSET;
        let asc = (doc_id / ASC_BLOCK_LEN) as u64 + noise;
...
fn build_shared_index(num_docs: usize, term_specs: &[(f64, TermLayout)]) -> BenchIndex {
...

    {
        ...
        for doc_id in 0..num_docs {
            ...
            numeric.add_to(&mut doc, &mut rng, doc_id);
...
fn main() {
    ...
    // stay curated, including the 18% term / 0.2% range pair that reproduces the regression.
    let scenarios = [
        (0.0001, 0.8),
        (0.001, 0.8),
        (0.01, 0.8),
        ...
        (0.5, 0.1),
        (0.7, 0.1),
        // Keeps the seek heuristic near its point-lookup/scan boundary.
        (0.0001, 0.0001),
        (0.001, 0.0001),
        (0.01, 0.0001),
        (0.1, 0.0001),
