...
fn test_write_commit_fails() -> tantivy::Result<()> {
    ...
    let mut index_writer: IndexWriter = index.writer_with_num_threads(1, 3_000_000)?;
    for _ in 0..100 {
...
fn test_fail_on_flush_segment() -> tantivy::Result<()> {
    ...
    let text_field = schema_builder.add_text_field("text", TEXT);
    let index = Index::create_in_ram(schema_builder.build());
    let index_writer: IndexWriter = index.writer_with_num_threads(1, 3_000_000)?;
...
fn test_fail_on_flush_segment_but_one_worker_remains() -> tantivy::Result<()> {
    ...
    let text_field = schema_builder.add_text_field("text", TEXT);
    let index = Index::create_in_ram(schema_builder.build());
    let index_writer: IndexWriter = index.writer_with_num_threads(2, 6_000_000)?;
