//! `FLOOR` returns the greatest integer less than or equal to one numeric argument.
//!
...
//! rounded toward negative infinity and converted to `i64`. Null, NaN, infinity, and any result
...
impl FnCall for FloorFnCall {
    ...
    fn emit_cranelift_ir(
        ...
        let (value, value_is_valid) = match self.arg.return_type {
            ...
            VarType::F64 => {
                let rounded = builder.ins().floor(arg.value);
...
impl From<FloorFnCall> for FnCallEnum {
    fn from(call: FloorFnCall) -> Self {
        FnCallEnum::Floor(call)
    }
}
...
mod tests {
    ...
    fn test_integer_identity_and_float_rounding() {
        assert_eq!(eval("(FLOOR -9223372036854775808i64)"), Some(i64::MIN));
        assert_eq!(eval("(FLOOR 9223372036854775807u64)"), Some(i64::MAX));
        assert_eq!(eval("(FLOOR 9223372036854775808u64)"), None);
        assert_eq!(eval("(FLOOR 1.2f64)"), Some(1));
        assert_eq!(eval("(FLOOR -1.2f64)"), Some(-2));
        assert_eq!(eval("(FLOOR -0f64)"), Some(0));
    }
    ...
    fn test_runtime_null() {
        ...
        assert_eq!(
            unsafe { compiled.call(&[VariableValue::some(2.9f64)]).as_i64() },
            Some(2)
        );
        // SAFETY: The compiled expression expects one nullable f64 argument.
        assert_eq!(
