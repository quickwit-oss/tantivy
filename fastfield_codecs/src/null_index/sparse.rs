...
/// The data starts with the raw data of u16 encoded positions of the blocks. [u16 LE, .. #repeat*; Desc: Positions with values in a block]
///
...
/// [u16 LE, .. #repeat*; Desc: Positions with values in a block][(u16 LE, u16 LE), .. #repeat*; Desc: (Block Id u16, Num Elements u16)][u16 LE; Desc: num blocks with
/// values u16]
///
/// # Opening
...
impl SparseCodecBlock {
    ...
    #[inline]
    fn value_at_idx<'a>(&self, data: &'a [u8], idx: u16) -> u16 {
    ...
    #[inline]
    fn iter_values<'a>(&'a self, data: &'a [u8]) -> impl Iterator<Item = u16> + 'a {
        let num_vals = self.num_vals;
        (0..num_vals).map(move |idx| self.value_at_idx(data, idx))
    }
